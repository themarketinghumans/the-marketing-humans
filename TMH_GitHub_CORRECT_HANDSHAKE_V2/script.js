...
// TMH handshake intro — uses the exact supplied JSON animation.
(() => {
  ...
  const finishIntro = () => {
    ...
    if (animation) {
      try { animation.stop(); } catch (_) {}
    }
    loader.classList.add('connection-complete');
    document.body.classList.remove('is-loading');
    document.body.classList.add('loaded');
    window.setTimeout(() => loader.remove(), reduce ? 0 : 780);
  };
  ...
  const startAnimation = () => {
    ...
    try {
      ...
      animation.addEventListener('DOMLoaded', () => {
        // The supplied file is 2s long but the actual handshake motion is
        // contained in the first second. Play only that clean handshake beat.
        try {
          animation.setSpeed(reduce ? 1.45 : 1);
      ...
      animation.addEventListener('complete', () => {
        window.setTimeout(finishIntro, reduce ? 0 : 160);
      });
  ...
  // Never trap visitors on the loader if a CDN/network issue occurs.
  fallbackTimer = window.setTimeout(finishIntro, 3600);
  waitForLottie();
})();
